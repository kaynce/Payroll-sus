...
namespace HRM
{
    public partial class Employee_Loan_Cash_Advance : Form
    {
        ...
        public Employee_Loan_Cash_Advance()
        {
            InitializeComponent();
            RetrieveData();
        ...
        private void RetrieveData()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * FROM P_Employee_Loan_Cash_Advance", con);
                ...
                dataGridView1.Columns[5].HeaderText = "Monthly Amortization"; dataGridView1.Columns[5].DataPropertyName = "Monthly_Amort";

                dataGridView1.Columns[6].HeaderText = "Semi-Monthly Amortization"; dataGridView1.Columns[6].DataPropertyName = "Semi_Monthly_Amort";
        ...
        private void cb_empidTxt_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrEmpty(cb_empidTxt.Text.Trim()))
                {
                    ...
                    SqlCommand cmd2 = new SqlCommand("SELECT [Surname], [Firstname] FROM P_Employee_List Where Employee_ID = @ID", con);
                    ...
                    while (dta2.Read())
                    {
                        ...
                        fnameTxt.Text = dta2.GetValue(1).ToString();
                    }
                    con.Close();
        ...
        private void saveBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlDataReader reader = null;
                SqlCommand cmd_Check = new SqlCommand("SELECT * FROM P_Employee_Loan_Cash_Advance", con);
                ...
                if (!string.IsNullOrEmpty(emp_ID) && !loanAmount.Equals("0.00"))
                {
                    if (!reader.HasRows)
                    {
                        ...
                        if (confirm.Equals(DialogResult.Yes))
                        {
                            ...
                            SqlCommand cmd = new SqlCommand("INSERT INTO P_Employee_Loan_Cash_Advance([Date], [Employee_ID]," +
                                "[Surname], [Firstname], [Loan_Amount], [Monthly_Amort], [Semi_Monthly_Amort], [Balance]) VALUES('" +
                                date + "','" + emp_ID + "','" + surname + "','" + fname + "','" + loanAmount +
                                "','" + monthlyAmort + "','" + semi_Amort + "','" + balance + "')", con);

                            cmd.ExecuteNonQuery();

                            SqlCommand cmd_History = new SqlCommand("INSERT INTO P_Employee_Loan_Cash_Advance_History([Date], [Employee_ID]," +
                                "[Surname], [Firstname], [Loan_Amount], [Monthly_Amort], [Semi_Monthly_Amort], [Balance]) VALUES('" +
                                date + "','" + emp_ID + "','" + surname + "','" + fname + "','" + loanAmount +
                                "','" + monthlyAmort + "','" + semi_Amort + "','" + balance + "')", con);
                ...
                {
                    cb_empidTxt.Select();
                    MessageBox.Show("Choose Employee ID", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                con.Close();
            }
            catch { }
