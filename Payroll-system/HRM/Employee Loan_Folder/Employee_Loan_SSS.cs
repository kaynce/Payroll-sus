...
namespace HRM
{
    public partial class Employee_Loan_SSS : Form
    {
        ...
        private void RetrieveData()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * FROM P_Employee_Loan_SSS", con);
        ...
        private void cb_empidTxt_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrEmpty(cb_empidTxt.Text.Trim()))
                {
                    ...
                    SqlCommand cmd2 = new SqlCommand("SELECT [SSS], [Surname], [Firstname] FROM P_Employee_List Where Employee_ID = @ID", con);
                    ...
                    while (dta2.Read())
                    {
                        sssTxt.Text = dta2.GetValue(0).ToString();
        ...
        private void saveBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlDataReader reader = null;
                SqlCommand cmd_Check = new SqlCommand("SELECT * FROM P_Employee_Loan_SSS", con);
                ...
                if (!string.IsNullOrEmpty(emp_ID) && !loanAmount.Equals("0.00"))
                {
                    if (!reader.HasRows)
                    {
                        ...
                        if (confirm.Equals(DialogResult.Yes))
                        {
                            ...
                            SqlCommand cmd = new SqlCommand("INSERT INTO P_Employee_Loan_SSS([Date], [Employee_ID]," +
                                "[SSS_No], [Surname], [Firstname], [Loan_Amount], [Monthly_Amort], [Semi_Monthly_Amort], [Balance]) VALUES('" +
                                date + "','" + emp_ID + "','" + sss + "','" + surname + "','" + fname + "','" + loanAmount +
                                "','" + monthlyAmort + "','" + semi_Amort + "','" + balance + "')", con);

                            cmd.ExecuteNonQuery();

                            SqlCommand cmd_History = new SqlCommand("INSERT INTO P_Employee_Loan_SSS_History([Date], [Employee_ID]," +
                                "[SSS_No], [Surname], [Firstname], [Loan_Amount], [Monthly_Amort], [Semi_Monthly_Amort], [Balance]) VALUES('" +
                                date + "','" + emp_ID + "','" + sss + "','" + surname + "','" + fname + "','" + loanAmount +
                                "','" + monthlyAmort + "','" + semi_Amort + "','" + balance + "')", con);
